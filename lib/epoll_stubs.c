...
intnat caml_errno(void)
{
	return (intnat)errno;
}

value caml_errno_byte(void)
{
	CAMLparam0();
	CAMLreturn(Val_int(caml_errno));
}

intnat caml_epoll_create1(void)
{
	return (intnat)epoll_create1(EPOLL_CLOEXEC);
}

value caml_epoll_create1_byte(void)
{
	CAMLparam0();
...
intnat caml_epoll_ctl(intnat epollfd, intnat op, value vfd, intnat flags)
{
	CAMLparam1(vfd);
...
value caml_epoll_ctl_byte(value vepollfd, value vop, value vfd, value vflags)
{
	CAMLparam4(vepollfd, vop, vfd, vflags);
	intnat ret = caml_epoll_ctl(Int_val(vepollfd), Int_val(vop), vfd,
				    Int_val(vflags));
	CAMLreturn(Val_int(ret));
}

intnat caml_epoll_wait(intnat epollfd, value vepollevents, intnat maxevents,
		       intnat timeout)
{
...
value caml_epoll_wait_byte(value vepollfd, value vepollevents, value vmaxevents,
			   value vtimeout)
{
	CAMLparam4(vepollfd, vepollevents, vmaxevents, vtimeout);
	intnat ret = caml_epoll_wait(Int_val(vepollfd), vepollevents,
				     Int_val(vmaxevents), Int_val(vtimeout));
	CAMLreturn(Val_int(ret));
}

value caml_accept4(value vfd)
{
	CAMLparam1(vfd);
	CAMLlocal1(a);
	int flags;
...
intnat caml_read(value vfd, value v_buf, intnat offset, intnat len)
{
...
value caml_read_byte(value v_fd, value v_buf, value v_ofs, value v_len)
{
